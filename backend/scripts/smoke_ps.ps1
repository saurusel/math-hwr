...
# --- 0) Консоль и файлы строго в UTF-8 ---
...
# --- 2) Чтение меток строго как UTF-8 ---
$labels = Get-Content -Encoding UTF8 "data/synth/val/labels.jsonl" -TotalCount $N | ForEach-Object { $_ | ConvertFrom-Json }
...
foreach($row in $labels){
    ...
    $decodeCfg = if ($DecodeType -eq "beam") { @{ type="beam"; beam_width=$BeamWidth } } else { @{ type="greedy" } }
    ...
    try {
        ...
        # только для отображения, если когда-то консоль «съест» ÷
        $pred_disp = $pred -replace 'Ã·','÷'

        $i++
        Write-Host ("[{0}/{1}] {2} -> {3}" -f $i, $labels.Count, $id, $pred_disp)
...
# безопасный "первые 10", без IndexOutOfRange при нуле
$first10 = if ($mismatches.Count -gt 0) { $mismatches | Select-Object -First 10 } else { @() }
